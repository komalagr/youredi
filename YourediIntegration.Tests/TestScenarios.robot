...
TC30: Bug 8955: Check that user can delete Endpoint bindings from sub-organization
    [Documentation]    Check that user can delete bindngsfrom sub organization
    ...
    ...
    ...    Test Data Setup
    ...    1. Need Organization Name : Main Org (2nd organization \ ) parent of Child Org(3rd Org)
    ...    2. Need Service -- MasterService
    Open Browser    ${login.url}    chrome
    Input Text    id=email    ${login.email}
    Input Password    id=password    ${login.password}
    Click Element    class=fa-sign-in
    Sleep    15s
    click element    xpath= //*[@id="webide"]/main/section/section/div/div[1]/div/p/a[1]/i    #click continue
    sleep    11s
...
TC54_DesignerRole
    ...
    Input Password    id=password    RobotTesting@456
    Click Element    class=fa-sign-in
    Sleep    12s
...
TC54_AdminRole
    ...
    Input Password    id=password    RobotTesting@789
    Click Element    class=fa-sign-in
    Sleep    12s
    Maximize Browser Window
    Page should contain    text=Komal test organistaion    # can see org and its sub org
...
TC54_AdminRoleWithGlobalAdminRights
    [Documentation]    Check that user that has Admin role but does not have Global admin rights, has full rights at account and service level but does not have access to admin tools
    Open Browser    https://stage.youredi.com/    chrome
    Input Text    id=email    globaladmintester@youredi.com
    Input Password    id=password    RobotTesting@890
    Click Element    class=fa-sign-in
    Sleep    15s
    Maximize Browser Window
    Page should contain    text=Komal test organistaion    # can see org and its sub org
